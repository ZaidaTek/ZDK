...
ZT_INDEX ZTW32_Process(void) {
	...
	while (PeekMessage(&(rZTW32_Host.window.message), NULL, 0, 0, PM_REMOVE)) {
...
void ZTW32_Quit(void) {SendMessageCallback(rZTW32_Host.window.handle, WM_CLOSE, 0x0, 0x0, NULL, 0);}
...
LRESULT CALLBACK ZTW32_CallbackMain(HWND iHwnd, UINT iMessage, WPARAM iWParam, LPARAM iLParam) {
	...
	if ((iMessage == ZTW32_WM_USER) || (lPumped = (iMessage == ZTW32_WM_PUMP ? ZT_TRUE : ZT_FALSE))) {
		// pass
		...
		case WM_CLOSE: ZTK_MSG_Close(); ZTW32_ModeExit(); break;
		case WM_CHAR: if (rZTK_Host.system.input.flag & ZTK_WINDOW_INPUT_CHAR) {ZTK_MSG_Char(iWParam & (~((ZT_CHAR)0x0)));} break; // TODO higher up when used
