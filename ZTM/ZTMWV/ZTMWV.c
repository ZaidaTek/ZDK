...
#define mZWV_EMPTY(TARGET) if ((TARGET) != NULL) {((ZT_U8*)(TARGET)->data.ptr)[((TARGET)->data.length = 0)] = ZTM_CHAR_NT;}
#define mZWV_ADD(TARGET,CONDITION) \
...
void ZWV_Empty(void) {mZWV_EMPTY(rTarget);}
void ZWV_Add(const void* iData) {mZWV_ADD(rTarget, (lAppend[++lRead] != ZTM_CHAR_NT));}
void ZWV_AddVerbatim(const void* iData, ZT_SIZE iLength) {mZWV_ADD(rTarget, (++lRead < iLength));}
...
#define mZWV_ADDNUMBER(NUMBER,FORMAT) ZT_U8* lNumber = FORMAT(NUMBER); ZWV_Add(lNumber); ZTM8_Free(lNumber)
void ZWV_AddInteger(ZT_I iInteger) {mZWV_ADDNUMBER(iInteger, ZTC8_Integer);}
void ZWV_AddUnsigned(ZT_U iInteger) {mZWV_ADDNUMBER(iInteger, ZTC8_Unsigned);}
void ZWV_AddHex(ZT_U iInteger) {mZWV_ADDNUMBER(iInteger, ZTC8_Hex);}
void ZWV_AddBin(ZT_U iInteger) {mZWV_ADDNUMBER(iInteger, ZTC8_Binary);}
#undef mZWV_ADDNUMBER
void ZWVX_Empty(ZT_WEAVE* iTarget) {mZWV_EMPTY(iTarget);}
void ZWVX_Add(ZT_WEAVE* iTarget, const void* iData) {mZWV_ADD(iTarget, (lAppend[++lRead] != ZTM_CHAR_NT));}
void ZWVX_AddVerbatim(ZT_WEAVE* iTarget, const void* iData, ZT_SIZE iLength) {mZWV_ADD(iTarget, (++lRead < iLength));}
...
#define mZWVX_ADDNUMBER(TARGET,NUMBER,FORMAT) ZT_U8* lNumber = FORMAT(NUMBER); ZWVX_Add((TARGET),lNumber); ZTM8_Free(lNumber)
void ZWVX_AddInteger(ZT_WEAVE* iTarget, ZT_I iInteger) {mZWVX_ADDNUMBER(iTarget, iInteger, ZTC8_Integer);}
void ZWVX_AddUnsigned(ZT_WEAVE* iTarget, ZT_U iInteger) {mZWVX_ADDNUMBER(iTarget, iInteger, ZTC8_Unsigned);}
void ZWVX_AddHex(ZT_WEAVE* iTarget, ZT_U iInteger) {mZWVX_ADDNUMBER(iTarget, iInteger, ZTC8_Hex);}
void ZWVX_AddBin(ZT_WEAVE* iTarget, ZT_U iInteger) {mZWVX_ADDNUMBER(iTarget, iInteger, ZTC8_Binary);}
#undef mZWVX_ADDNUMBER
#undef mZWV_ADD
#undef mZWV_EMPTY
